...
contract GasLZV2 is OptimizedOApp {
    ...
    function sendMessages(uint32[] calldata _dstEids, bytes[] calldata _messages) external payable {
        uint256 fee;
    ...
    function sendDeposits(uint256[] calldata _depositParams, address _to) external payable {
        uint256 fee;
        for (uint256 i; i < _depositParams.length; i++) {
    ...
    function _sendDeposit(
        ...
        MessagingReceipt memory receipt = _lzSend(
            _dstEid,
            "",
            createNativeDropOption(_dstEid, _amount, _to),
            address(this).balance,
            address(this)
        );
        return receipt.fee.nativeFee;
    ...
    function createNativeDropOption(
        uint32 _dstEid,
    ...
    function quote(
        ...
        MessagingFee memory fee = endpoint.quote(
            MessagingParams(_dstEid, getPeer(_dstEid), _message, _options, false),
            address(this)
        );
